...
<body>
	<div class="appendix-no"><b>Form 1</b></div>
	<br><br><br><br>
		...
		<tbody>
		    ...
		    @foreach ($itemConsolidate as $relatedItem)
		        <tr>
		            ...
		            <td>{{ $relatedItem->item_descrip }}</td>
		            <td>{{ $relatedItem->qty }}</td>
		            <td>{{ $relatedItem->item_cost }}</td>
		            <td align="right"><b>{{ number_format($relatedItem->total_cost, 2) }}</b></td>
            ...
            <tr>
				<th colspan="5" class="cell-bold-total-left" style="text-align: left;"><b>Grand Total</b></th>
			...
			<tr>
				...
				<th colspan="5" class="cell-bold-total-right" style="text-align: left; font-size: 8pt;">
					@foreach($itemConsPR as $itempr)
						{{ $itempr->pr_no}}, 
					@endforeach
				</th>
			</tr>
			<tr>
				...
				<th colspan="3" class="cell-approved-designation">{{ $currentDate }}</th>
			</tr>
		</tbody>
	</table>
</body>
